...
DROP_PHENOTYPE_PCS = [
    'Whole_blood': [25] + (51..100),
    'Whole_blood___EUR': [19, 25]
]
...
process apply_permutations {
    ...
    input:
    tuple val(tissue), path(pairs), path(permutations_pickle)

    output:
    tuple val(tissue), path("${tissue}.trans_qtl.top.txt")

    """
    trans-eqtl-filter-apply-permutations-fdr-correct.py --pairs $pairs --permutations $permutations_pickle > ${tissue}.trans_qtl.top.txt
    """
...
workflow trans {
    ...
    top = trans_out_with_mappability.combine(trans_permutations.permutations_pickle, by: 0) | apply_permutations
