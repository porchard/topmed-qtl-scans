...
# get the genes w/in 5Mb of each variant
...
#within_5Mb = bt.BedTool().from_dataframe(variant_bed).sort().window(b=bt.BedTool().from_dataframe(tss).sort(), w=int(5e6)).to_dataframe()
# break down by chrom for efficiency
CHROMS = [f'chr{i}' for i in range(1, 23)] + ['chrX']
assert(all(variant_bed.chrom.isin(set(CHROMS))))
within_5Mb = []
for chrom in CHROMS:
    within_5Mb_chrom = bt.BedTool().from_dataframe(variant_bed[variant_bed.chrom==chrom]).sort().window(b=bt.BedTool().from_dataframe(tss[tss.chrom==chrom]).sort(), w=int(1e6)).to_dataframe()
    within_5Mb.append(within_5Mb_chrom[['name', 'thickEnd']].drop_duplicates())
within_5Mb = pd.concat(within_5Mb)



variant_to_nearby_genes_5Mb = {variant: set() for variant in trans.variant_id.unique()}
for variant, gene in zip(within_5Mb['name'], within_5Mb['thickEnd']):
    variant_to_nearby_genes_5Mb[variant].add(gene)

def gene_crossmaps_to_gene_near_variant(phenotype_id, variant_id):
    nearby_genes = variant_to_nearby_genes_5Mb[variant_id]
...
trans['gene_mappability'] = trans.phenotype_id.map(gene_mappability)
trans['gene_crossmaps_to_gene_near_variant'] = [gene_crossmaps_to_gene_near_variant(phenotype_id, variant_id) for variant_id, phenotype_id in zip(trans.variant_id, trans.phenotype_id)]
trans['biotype'] = trans.phenotype_id.map(gene_id_to_biotype)

trans.to_csv(sys.stdout, sep='\t', index=False)


# filtered = trans[(trans.gene_mappability>=0.8) & (~trans.gene_crossmaps_to_gene_near_variant) & (trans.snp_mappability>=1.0)]
# logging.info('{:,} of {:,} rows remain after filtering'.format(len(filtered), len(trans)))
